...
# limit how much memory JAX/XLA grabs before we import anything that pulls in ott/jax
...
# Prefer the vendored OTT sources (with label support) over the installed package.
sys.path.insert(0, os.path.join(current_dir, "ott", "src"))
...
def train(epoch,train_loader,model, T_feature_2, best_acc=0.0):
    ...
    for batch_idx, (data, target) in enumerate(tqdm(train_loader)):
        ...
        # Pad batch to fixed size to prevent JAX recompilation
        ...
        if current_batch_size < max_batch_size:
            ...
            padded_data = []
            for v_num in range(len(data)):
                pad_shape = [pad_size] + list(data[v_num].shape[1:])
                padding = torch.zeros(pad_shape, device=data[v_num].device, dtype=data[v_num].dtype)
                padded_data.append(torch.cat([data[v_num], padding], dim=0))
            data = padded_data
            
            # Pad target with a dummy class (use 0, will be masked)
            padding_target = torch.zeros(pad_size, device=target.device, dtype=target.dtype)
            target_padded = torch.cat([target, padding_target], dim=0)
        else:
            target_padded = target
            
        data_num += current_batch_size  # Only count real samples
        
        # refresh the optimizer
        optimizer.zero_grad()
        pred, loss, ot_loss = model(X=data, y=target_padded, T_feature_2=T_feature_2, training=True)
        
        # Slice predictions and loss back to original size
    ...
    save_results(r'/home/prml/RIMA/datasets/DR/results/results.txt', epoch, loss_meter, aver_acc, precision, recall, f1, auc, kappa, specificity)
    return loss_meter
...
def group_features_by_label(y, p, num_classes=3, max_samples_per_label=None):
    """
    y: [N] labels
    p: [N, ...] features
    return: dictionary mapping label -> numpy array of features, optionally capped per label
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no-pretrained', action='store_true',
                        help='Train from scratch without loading pretrained weights')
    ...
    parser.add_argument('--max-jax-samples', type=int, default=128,
                        help='max features per label sent to OTT to keep JAX compile size bounded')
    ...
    # args.base_path = r'D:\AMD\mnt\sdb\feilong\Retinal_OCT\Medical_data\\train/'
    # args.data_path = r'D:\AMD\mnt\sdb\feilong\Retinal_OCT\Medical_data\AMD\train\Image'
    # args.base_path = r'/home/prml/RIMA/datasets/DR/'
    # args.data_path = r'/home/prml/RIMA/datasets/DR/Image_new/'
    ...
    for epoch in range(args.start_epoch, args.end_epochs + 1):
        ...
        grouped_fudus_feature = group_features_by_label(
            train_labels,
            fundus_features.cpu().detach().numpy(),
            max_samples_per_label=args.max_jax_samples,
        )
        grouped_oct_feature = group_features_by_label(
            train_labels,
            oct_features.cpu().detach().numpy(),
            max_samples_per_label=args.max_jax_samples,
        )
            # fundus2oct
